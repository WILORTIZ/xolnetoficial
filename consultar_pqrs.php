...
if (!empty($searchId)) {
    ...
    try {
        if ($pdo instanceof PDO) {
            ...
            $errorMessage = "No hay conexión a la base de datos para consultar el radicado.";
        }
    } catch (PDOException $e) {
        $errorMessage = "Error al consultar la solicitud: " . $e->getMessage();
    }
}

$pageTitle = "Consultar PQRS";
include 'header.php';
?>
...
<section class="py-16 md:py-24 bg-surface min-h-[80vh]">
    <div class="max-w-container-max mx-auto px-4 md:px-margin-desktop">
        <div class="text-center max-w-2xl mx-auto mb-12">
            <span class="font-label-md text-label-md text-primary uppercase tracking-[0.2em] mb-3 block font-semibold">Seguimiento de Radicado</span>
            <h1 class="font-display text-display text-3xl md:text-5xl font-bold text-on-surface mb-4">Consultar Estado de PQRS</h1>
            <p class="font-body-lg text-on-surface-variant text-base md:text-lg">
                Ingresa el número de radicado proporcionado al registrar tu solicitud para verificar su estado en tiempo real.
            </p>
        </div>

        <div class="max-w-2xl mx-auto mb-10">
            <div class="glass rounded-xl p-6 md:p-8 border border-outline-variant/40 shadow-xl">
                <form action="consultar_pqrs.php" method="get" class="flex flex-col sm:flex-row gap-4">
                    <div class="flex-1 space-y-1">
                        <label for="id" class="sr-only">Número de Radicado</label>
            ...
            <div class="max-w-2xl mx-auto">
                <div class="glass rounded-xl p-8 border border-outline-variant/40 shadow-2xl relative text-left">
                    <div class="flex items-center justify-between border-b border-outline-variant/20 pb-6 mb-6">
                        <div>
                            <span class="font-mono text-label-md text-outline">NÚMERO DE RADICADO</span>
                            <h3 class="font-display text-2xl font-bold text-on-surface mb-0">#<?php echo htmlspecialchars($pqrs['Id'] ?? $pqrs['id']); ?></h3>
                        </div>
                        <?php 
                            $estado = $pqrs['Estado'] ?? $pqrs['estado'] ?? 'Pendiente';
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                        <div>
                            <p class="font-label-md text-xs font-semibold uppercase tracking-wider text-outline mb-1">Nombre Solicitante</p>
                            <p class="font-body-md text-on-surface font-semibold mb-0"><?php echo htmlspecialchars($pqrs['Nombre'] ?? $pqrs['nombre']); ?></p>
                        </div>
                        <div>
                            <p class="font-label-md text-xs font-semibold uppercase tracking-wider text-outline mb-1">Correo Electrónico</p>
                            <p class="font-body-md text-on-surface font-semibold mb-0"><?php echo htmlspecialchars($pqrs['Email'] ?? $pqrs['email']); ?></p>
                        </div>
                        <div>
                            <p class="font-label-md text-xs font-semibold uppercase tracking-wider text-outline mb-1">Tipo de Solicitud</p>
                            <p class="font-body-md text-on-surface font-semibold mb-0"><?php echo htmlspecialchars($pqrs['Tipo'] ?? $pqrs['tipo']); ?></p>
                        </div>
                        <div>
                            <p class="font-label-md text-xs font-semibold uppercase tracking-wider text-outline mb-1">Fecha de Registro</p>
                            <p class="font-body-md text-on-surface font-semibold mb-0"><?php echo htmlspecialchars($pqrs['FechaCreacion'] ?? $pqrs['fechacreacion']); ?></p>
                        </div>
                    </div>

                    <div class="border-t border-outline-variant/20 pt-6">
                        <p class="font-label-md text-xs font-semibold uppercase tracking-wider text-outline mb-2">Mensaje Radicado</p>
                        <div class="p-4 bg-surface-container-low rounded-lg border border-outline-variant/30 text-on-surface-variant font-body-md leading-relaxed">
                            <?php echo nl2br(htmlspecialchars($pqrs['Mensaje'] ?? $pqrs['mensaje'])); ?>
                        </div>
                    </div>
                </div>
            </div>
